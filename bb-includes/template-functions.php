...
function topic_class( $class = '', $key = 'topic', $id = 0 ) {
	...
	if (!is_topic() && !is_forum()) 
		$classes[] = "in-forum-" . $topic->forum_id;
...
function post_ip_link( $post_id = 0, $class = 'post-link' ) {
	if ( !bb_current_user_can( 'view_by_ip' ) )
		return null;
	$link = '<a href="' . attribute_escape( bb_get_option('uri') . 'bb-admin/view-ip.php?ip=' . get_post_ip( $post_id ) ) . '" class="' . $class . '">' . get_post_ip( $post_id ) . '</a>';
	return apply_filters( 'post_ip_link', $link, get_post_id( $post_id ) );
}

function post_edit_link( $post_id = 0, $class = 'post-link' ) {
	$bb_post = bb_get_post( get_post_id( $post_id ) );
	if ( bb_current_user_can( 'edit_post', $bb_post->post_id ) )
		return "<a href='" . attribute_escape( apply_filters( 'post_edit_uri', bb_get_option('uri') . 'edit.php?id=' . $bb_post->post_id, $bb_post->post_id ) ) . "' class='" . $class . "'>". __('Edit') ."</a>";
...
function post_delete_link( $post_id = 0, $class = 'post-link' ) {
	$bb_post = bb_get_post( get_post_id( $post_id ) );
	if ( !bb_current_user_can( 'delete_post', $bb_post->post_id ) )
		return null;

	if ( 1 == $bb_post->post_status )
		$r = "<a href='" . attribute_escape( bb_nonce_url( bb_get_option('uri') . 'bb-admin/delete-post.php?id=' . $bb_post->post_id . '&status=0&view=all', 'delete-post_' . $bb_post->post_id ) ) . "' onclick='return confirm(\" ". js_escape( __('Are you sure you wanna undelete that?') ) ." \");' class='" . $class . "'>". __('Undelete') ."</a>";
	else
		$r = "<a href='" . attribute_escape( bb_nonce_url( bb_get_option('uri') . 'bb-admin/delete-post.php?id=' . $bb_post->post_id . '&status=1', 'delete-post_' . $bb_post->post_id ) ) . "' onclick='return ajaxPostDelete(" . $bb_post->post_id . ", \"" . get_post_author( $post_id ) . "\", this);' class='" . $class . "'>". __('Delete') ."</a>";
...
function user_update_button() {
    echo apply_filters('user_update_button', get_user_update_button());
}
...
do_action('bb_profile_admin_form', $user->ID);
...
function bb_profile_password_form( $id = 0 ) {
	...
	if ( $message = $errors->get_error_message( 'pass' ) ) {
		$class .= ' error form-invalid';
		$message = '<p class="help-block error">' . wp_specialchars( $message ) . '</p>';
	}
?>
...
function bb_get_tag_remove_link($class = 'tag_remove_link') {
	global $tag, $topic;
	...
	$r = '<a class="' . $class . '"href="' . attribute_escape( bb_nonce_url( $url, 'remove-tag_' . $tag->tag_id . '|' . $tag->topic_id) ) . '" onclick="return ajaxDelTag(' . $tag->tag_id . ', ' . $tag->user_id . ', \'' . js_escape($tag->raw_tag) . '\', this);" title="' . attribute_escape( __('Remove this tag') ) . '">&times;</a>';
	return $r;
}
