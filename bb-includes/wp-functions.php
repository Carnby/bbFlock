...
function wp_specialchars( $string, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false )
{
	...
	// Store the site charset as a static to avoid multiple calls to bb_get_option()
	if ( !$charset ) {
		static $_charset;
		if ( !isset( $_charset ) ) {
			$_charset = bb_get_option( 'charset' );
		}
		$charset = $_charset;
	}
...
function wp_check_invalid_utf8( $string, $strip = false )
{
	...
	static $is_utf8;
	if ( !isset( $is_utf8 ) ) {
		$is_utf8 = in_array( bb_get_option( 'charset' ), array( 'utf8', 'utf-8', 'UTF8', 'UTF-8' ) );
	}
	if ( !$is_utf8 ) {
		return $string;
	}
...
if ( !function_exists('make_clickable') ) : // [WP12250]
...
if ( !function_exists( '_make_url_clickable_cb' ) ) : // [WP12088]
...
if ( !function_exists( '_make_web_ftp_clickable_cb' ) ) : // [WP11844]
...
if ( !function_exists( '_make_email_clickable_cb' ) ) : // [WP6449]
